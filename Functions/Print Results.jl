...
function print_formatted_results(solution::Dict, pm)
    ...
    max_vuf = maximum(results_df.vuf_percent)
    printstyled("\nMaximum VUF (%): ", round(max_vuf, digits=5) ; color = :yellow)
...
scatter!(p2, bar_positions[1:3:end], results_df.Zlin_percent[1:3:end], 
        color=:red, markersize=9, alpha=0.7, marker = :square, label="Modified PVUR (%)")
