...
# Create a DataFrame with each phase as a column
df = DataFrame(
    Phase_1 = active_shadow_prices_adjusted[1],
    Phase_2 = active_shadow_prices_adjusted[2],
    Phase_3 = active_shadow_prices_adjusted[3]
)
...
for phase in 1:3
    ...
    df_CSV = DataFrame(x = buses_adjusted)

    # Add each phase's y-values as a separate column
    for phase in 1:3
        phase_name = phase_map[phase]
        y_values = active_shadow_prices_adjusted[phase]
        df_CSV[!, "Phase_$phase_name"] = y_values

        # Plot as before
        plot!(active_plot2, buses_adjusted, y_values,
            label = "Phase $phase_name",
            marker = marker_map[phase],
            linestyle = :solid,
            linewidth = 2,
    ...
    separate_filename_3 = joinpath(output_dir, "active_shadow_prices$(separate_ext).csv")

    # Save the DataFrame to a CSV file
    CSV.write(separate_filename_3, df_CSV)
    println("Active shadow prices data saved to: $separate_filename_3")
